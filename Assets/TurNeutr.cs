...
public class TurNeutr : MonoBehaviour
{
    float maxValue;
    ...
    void Update()
    {
        ...
        {
            if (((goArray[i].transform.position - transform.position).sqrMagnitude <= 750)&& (goArray[i].transform.position != transform.position))
            {
                GameObject cur;
                hod = hod + 1;
                if ((hod == 1) && (Time.time - firet > 0.1))
                {
                    firet = Time.time;
