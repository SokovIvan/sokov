...
public class ScSp : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        life = 500;
        px = Random.Range(-80, 80);
        py = Random.Range(-80, 80);
        hod = 0;
        zam = 0;
    }
    ...
    void Update()
    {
        if (life <= 0)
        {
            ...
            Instantiate(gameObject, Base.transform.position, Quaternion.Slerp(transform.rotation, transform.rotation, 1));
            Destroy(gameObject);
        }
        float angle;
        hod = 0;
